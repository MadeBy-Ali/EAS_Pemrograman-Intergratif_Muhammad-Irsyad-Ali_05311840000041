...
class sumbangan extends Model
{
    ...
    public static function isThere($tipeSumbang)
    {

        try {
            $db = static::getDB();
            $stmt = $db->query("SELECT `id` FROM `jenis_sumbangan` WHERE `name`='".$tipeSumbang."' LIMIT 1");
    ...
    public static function setJS($tipeSumbang)
    {

        try {
            $db = static::getDB();
            $stmt = $db->query("INSERT INTO `jenis_sumbangan`(`name`) VALUES ('". $tipeSumbang ."')");
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            return $db->lastinsertId();
